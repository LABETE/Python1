...
gettysburg = """\
Four score and seven years ago our deinstitutionalizingsdfsd
...
for line in lines:
    ...
    wordct += len(words)
    for word in words:
        while len(lengthct) <= len(word):
            lengthct.append(0)
        lengthct[len(word)] += 1
        if not [word,words.count(word)] in lstwords:
            lstwords.append([word,gettysburg.count(" " + word + " ")]) 
print("The text contains", linect, "lines,", wordct, "words, and", charct, "characters.")
for i, ct in enumerate(lengthct):
    if ct:
        print("Length", i, ":", ct)
for i, ct in enumerate(lstwords):
    print(ct)
print 